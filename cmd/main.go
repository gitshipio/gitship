...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "2e404b74.gitship.io",
	...
	// UserReconciler removed as redundant/invalid
	...
	// Register Webhook Receiver (using simple HTTP for local dev/simpler setup)
	// mgr.GetWebhookServer().Register("/webhook", &gitshipwebhook.Receiver{Client: mgr.GetClient()})

	// Start a separate HTTP server for the webhook to avoid TLS headache locally
	go func() {
		setupLog.Info("Starting Webhook HTTP Server on :3001")
		http.Handle("/api/webhooks/github", &gitshipwebhook.Receiver{Client: mgr.GetClient()})
